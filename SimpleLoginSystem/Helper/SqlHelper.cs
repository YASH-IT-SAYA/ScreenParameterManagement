...
namespace ArkaPRP83.Core.Helper
{
    /// <summary>
    ...
    public sealed class SqlHelper
    {
        ...
        //public static string ConnString = ConfigurationManager.ConnectionStrings["ArkaPrn4Entities"].ConnectionString;
        ...
        private static bool StartRequiredServices()
        {
            try
            {
                List<ServiceController> serviceControllers;
                serviceControllers = ServiceController.GetServices().Where(m => m.ServiceName.Contains("SQL")).ToList();
                foreach (ServiceController serviceController in serviceControllers)
                {
                    //MSSQL//if (serviceController.ServiceName.Contains("SQL Server (SQLEXPRESS)") || serviceController.DisplayName.Contains("SQL Server Browser"))
                    if (serviceController.ServiceName.Contains("MSSQL") || serviceController.ServiceName.Contains("SQLBrowser"))
        ...
        internal static DataSet ExecuteDataset(object connString, CommandType storedProcedure, string v, SqlParameter[] sqlParameter)
        {
        ...
        public static int ExecuteNonQuery(string connectionString, CommandType commandType, string commandText, params SqlParameter[] commandParameters)
        {
            int count = 0;
            jumphere:
            if (connectionString == null || connectionString.Length == 0) throw new ArgumentNullException("Connection String");
            ...
            try
            {
                ...
                {
                    connection.Open();
                    if (connection == null || connection.State == ConnectionState.Closed || connection.State == ConnectionState.Broken) throw new Exception("Database Connection Error");
            ...
            {
                if (sex.Class == 20 && count <= 2)
                {
                    if (StartRequiredServices())
                        count++;
                    goto jumphere;
                }
                else
        ...
        /// <param name="parameterValues">An array of objects to be assigned as the input values of the stored procedure</param>
        /// <returns>An object containing the value in the 1x1 resultset generated by the command</returns>
        public static object ExecuteScalar(string connectionString, CommandType commandType, string spName, params object[] parameterValues)
        {
            if (connectionString == null || connectionString.Length == 0) throw new ArgumentNullException("Connection String");
            if (spName == null || spName.Length == 0) throw new ArgumentNullException("spName");
